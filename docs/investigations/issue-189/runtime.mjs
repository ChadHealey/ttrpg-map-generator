...
const privateFiles = ['bridge.ts'];
const allowed = (file) =>
  /^(packages\/(core|generation)\/src\/)[a-z0-9-/]+\.ts$/.test(file) ||
  privateFiles.some((name) => file === `docs/investigations/issue-189/${name}`);
...
export async function runtimeSources() {
  ...
  const pending = ['docs/investigations/issue-189/bridge.ts'],
    sources = {};
...
export async function loadRuntime(snapshot) {
  ...
  const directory = await mkdtemp(join(tmpdir(), 'issue189-public-'));
  try {
    ...
    const bridge = await import(
      pathToFileURL(join(directory, 'docs/investigations/issue-189/bridge.js')).href
    );
    ...
    return { bridge, generation, core, close: () => rm(directory, { recursive: true }) };
  } catch (error) {
