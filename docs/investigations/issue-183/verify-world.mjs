...
export async function verifyEvidence(
  ...
  assert.equal(manifest.revision, 'issue-183-world-r1-recipe3', 'unsupported revision');
  assert(
  ...
  for (const required of [
    'run.mjs',
    'recipes/recipe-3/templates.mjs',
  ...
  return {
    ...
    changedSources,
    scope:
      'Source text, dependency closure, condensed134-gate consistency, row arithmetic and PNG bytes verified. Full134 certificates, scalar grids, constructor repeats, world rerenders, cross-platform equality and human visual acceptance were not independently rerun or established.',
  };
}
