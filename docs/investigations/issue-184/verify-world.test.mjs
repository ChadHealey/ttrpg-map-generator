  ...
  probes().map(
    async (probe) =>
      JSON.parse(
        gunzipSync(
          await readFile(
            new URL(`../issue-183/readiness-recipe-3/${probe.input.id}.json.gz`, import.meta.url),
          ),
        ),
      ).result,
  ),
...
async function fixture({ comparison = true } = {}) {
  const dir = await mkdtemp(join(tmpdir(), 'issue184-world-verify-'));
  temporary.push(dir);
  const sourceRoot = join(dir, 'issue-184'),
  ...
  const snapshot = {
    ...
    'design.md': 'Declared bounded construction.\n',
    'state-2.md': 'Synthetic declared state.\n',
    '../issue-178/design.md': 'Frozen certificate authority.\n',
    '../issue-164/visual-contract.md': 'Frozen visual authority.\n',
  ...
  const gate = {
    ...
    layouts: [0, 1, 2],
    recoveredLayouts: [3],
    recoveredRows: inputs.slice(6).map(({ id }) => ({ id, primaryLayouts: [3] })),
    rows: local.map((r) => ({
      id: r.probe.input.id,
...
describe('independent nine-row world evidence verification', () => {
  ...
  it('closes a comparison after any failed gate row and forbids results or images', async () => {
    const f = await fixture({ comparison: false });
    f.manifest.kind = 'comparison';
    await f.save('manifest.json', f.manifest);
    const row = f.gate.rows[10];
    row.status = 'audit-failure';
    row.issues = [{ code: 'synthetic-placement-audit-failure' }];
    await f.save('local-gate.json', f.gate);
    await expect(f.verify()).rejects.toThrow('gate failure inventory');
    f.gate.failures = [{ code: 'failed-probe', id: row.id, status: row.status }];
    f.gate.readyForComparison = false;
    f.gate.completeCohortPassed = false;
    await f.save('local-gate.json', f.gate);
    expect(await f.verify()).toMatchObject({ gateRows: 134, imageCount: 0 });
    ...
    await f.save('results.json', f.results);
    await expect(f.verify()).rejects.toThrow('unexpected comparison results after local rejection');
  });

  it('requires every named recovered row to retain an actual large primary', async () => {
    const f = await fixture({ comparison: false });
    const row = f.gate.rows.find((entry) => entry.id === 'default-004');
    row.owners.find((owner) => owner.primary).layoutIndex = 1;
    await f.save('local-gate.json', f.gate);
    await expect(f.verify()).rejects.toThrow('gate recovered row inventory');
    f.gate.recoveredRows[1].primaryLayouts = [1];
    f.gate.recoveredLayouts = [1, 3];
    await f.save('local-gate.json', f.gate);
    await expect(f.verify()).rejects.toThrow('gate failure inventory');
    f.gate.failures = [{ code: 'missing-recovered-layout', id: 'default-004' }];
    f.gate.readyForComparison = false;
    f.gate.completeCohortPassed = false;
    await f.save('local-gate.json', f.gate);
    expect(await f.verify()).toMatchObject({ gateRows: 134, imageCount: 0 });
  ...
  it('limits revisions to two and binds R2 to the retained complete R1 receipt and design', async () => {
    const f = await fixture();
    f.manifest.revision = 'issue-184-world-r3';
    await f.save('manifest.json', f.manifest);
    await expect(f.verify()).rejects.toThrow('unsupported revision');
    f.manifest.revision = 'issue-184-world-r2';
    f.results.revision = f.manifest.revision;
    await f.save('manifest.json', f.manifest);
    await f.save('results.json', f.results);
    await expect(f.verify()).rejects.toThrow('missing required captured source: revision-r2.md');
    f.snapshot['revision-r2.md'] = 'Declared second bounded revision.\n';
    f.manifest.sources['revision-r2.md'] = hash(f.snapshot['revision-r2.md']);
    await writeFile(join(f.sourceRoot, 'revision-r2.md'), f.snapshot['revision-r2.md']);
    await f.save('source-snapshot.json', f.snapshot);
    const priorDir = join(f.sourceRoot, 'comparison-r1');
    await mkdir(priorDir);
    const priorManifest = JSON.stringify({
      revision: 'issue-184-world-r1',
      kind: 'comparison',
      inputs,
    });
    const priorResults = JSON.stringify({
      revision: 'issue-184-world-r1',
      reports: inputs.map((input) => ({ input })),
    });
    await writeFile(join(priorDir, 'manifest.json'), priorManifest);
    await writeFile(join(priorDir, 'results.json'), priorResults);
    f.manifest.predecessor = {
      revision: 'issue-184-world-r1',
      manifestSha256: hash(priorManifest),
      resultsSha256: hash(priorResults),
    };
    await f.save('manifest.json', f.manifest);
    await f.save('results.json', f.results);
    expect(await f.verify({ requireCurrent: true })).toMatchObject({
      predecessorReceiptVerified: true,
    });
    await writeFile(join(priorDir, 'results.json'), priorResults + '\n');
    await expect(f.verify()).rejects.toThrow('retained R1 predecessor receipt');
    const missing = JSON.stringify({
      revision: 'issue-184-world-r1',
      reports: inputs.slice(1).map((input) => ({ input })),
    });
    await writeFile(join(priorDir, 'results.json'), missing);
    f.manifest.predecessor.resultsSha256 = hash(missing);
    await f.save('manifest.json', f.manifest);
    await expect(f.verify()).rejects.toThrow('predecessor complete rows');
  });
