...
const privateFiles = ['policy.ts', 'zero-contours.ts', 'zero-geometry.ts', 'correspondence.ts'];
const allowed = (file) =>
  /^(packages\/(core|generation)\/src\/)[a-z0-9-/]+\.ts$/.test(file) ||
  privateFiles.some((name) => file === `docs/investigations/issue-186/${name}`);
...
export async function runtimeSources() {
  ...
  const pending = ['docs/investigations/issue-186/policy.ts'],
    sources = {};
...
export async function loadRuntime(snapshot) {
  ...
  const directory = await mkdtemp(join(tmpdir(), 'issue186-public-'));
  try {
    ...
    const policy = await import(
      pathToFileURL(join(directory, 'docs/investigations/issue-186/policy.js')).href
    );
