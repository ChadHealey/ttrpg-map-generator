/** Issue-191 mirror of issue-170 placement; only stream injection differs. Budgets and candidate order are frozen in design.md. */
...
function stream(context, ownerId, attempt, scope) {
  const suffix =
    scope === 'spherical-code-rotation'
      ? `placement.attempt${attempt}.codeRotation`
      : `placement.attempt${attempt}.${ownerId}.${
          {
            'center-directions': 'centerDirections',
            'feasible-displacements': 'refinement',
            orientation: 'orientation',
          }[scope]
        }`;
  const limit = {
    'spherical-code-rotation': 3,
    'center-directions': 256,
    'feasible-displacements': 64,
    orientation: 1,
  }[scope];
  return context.stream(suffix, limit);
}
