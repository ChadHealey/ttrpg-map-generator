...
async function fixture({ comparison = true, failedRow = false } = {}) {
  const directory = await mkdtemp(join(tmpdir(), 'issue-179-verifier-'));
  temporary.push(directory);
  const sourceRoot = join(directory, 'issue-179'),
  ...
  const reports = inputs.map((input) => ({
    ...
    construction: {
      ...
      owners: Array.from({ length: input.controls.continentCountIntent }, (_, i) => ({
        ...
        radius: 0.3,
        certificate: { ok: true, metrics: { collarWidthUpperMode: 'root-and-far' } },
...
describe('issue-179 independent read-only evidence verification', () => {
  ...
  it('requires a separate r2 runner and revision record without inventing constructor filenames', async () => {
    const evidence = await fixture({ comparison: false });
    evidence.manifest.revision = 'issue-179-r2';
    await evidence.save('manifest.json', evidence.manifest);
    ...
    expect(await evidence.verify({ requireCurrent: true })).toMatchObject({
      revision: 'issue-179-r2',
      verified: true,
    });
  });
  it('rejects missing or fallback certificate modes on accepted owners', async () => {
    const evidence = await fixture({ comparison: false });
    const metrics = evidence.gate.reports[0].construction.owners[0].certificate.metrics;
    for (const value of [undefined, 'root']) {
      metrics.collarWidthUpperMode = value;
      await evidence.save('local-gate.json', evidence.gate);
      await expect(evidence.verify()).rejects.toThrow('declared root-and-far certificate mode');
    }
  });
});
