...
import { constructTemplate as frozenTemplate } from '../issue-179/templates-r2.mjs';
...
/** First twelve calls delegate exactly to frozen179, including validation and seed streams. */
export function constructTemplate(options) {
  const index = options?.templateIndex ?? 0;
  if (Number.isInteger(index) && index >= 0 && index < 12) return frozenTemplate(options);
