/** Private extraction-2 regular-domain copy of atlas-coastline-contours.ts. Caller must reject every zero anchor and alternating determinant tie first. */
/**
...
import { atlasPlanetTopologyValidationAdapter } from '@ttrpg-map/generation';
import {
...
export function extractRegularZeroContours(
  ...
  addPolarBandSegments(field, 'south', segments, seenSegments, diagnostics);
  for (
    let latitudeIndex = 1;
    ...
    for (
      ...
      addQuadSegments(field, longitudeIndex, latitudeIndex, segments, seenSegments, diagnostics);
    }
  }
  addPolarBandSegments(field, 'north', segments, seenSegments, diagnostics);
...
function addQuadSegments(
  ...
  const signs = values.map((value) => value * 2);
...
function addPolarBandSegments(
  ...
  for (
    ...
    const signs = [westValue * 2, eastValue * 2, poleValue * 2] as const;
    const vertices: readonly CellVertex[] = [
      vertex(
        westLongitude,
        rowLatitude,
