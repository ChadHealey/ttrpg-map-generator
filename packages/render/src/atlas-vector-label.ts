...
export function validateAndExpandAtlasVectorLabelLayer(
  ...
  for (const candidate of layer.nodes as readonly unknown[]) {
    ...
    previousNode = node;
    for (const [index, glyph] of node.glyphs.entries()) {
  ...
  let expanded: readonly RenderCompoundPath[];
  try {
    expanded = expandAtlasVectorLabelLayer(layer);
  } catch {
    return invalid(
      'atlas-vector-label.geometry.invalid',
  ...
  for (const [index, renderNode] of expanded.entries()) {
    ...
    if (
      labelNode === undefined ||
      renderNode.subpaths.some((subpath) =>
        subpath.points.some(
          ({ xPx, yPx }) =>
            xPx < 0 ||
            xPx > 2_048 ||
  ...
  return (
    ...
    input.glyphs.every(
      (glyph) =>
        ...
        typeof glyph.originXTicks === 'number' &&
        typeof glyph.originYTicks === 'number',
    )
  );
}
