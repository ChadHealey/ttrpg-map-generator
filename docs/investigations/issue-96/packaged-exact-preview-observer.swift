...
let exactFixturePreviewObserverSchemaVersion = "packaged-exact-fixture-preview-observer-v1"
...
@main
enum PackagedExactPreviewObserver {
  static func main() async {
    do {
      let arguments = try ExactFixturePreviewObserverArguments(CommandLine.arguments)
      let receipt = try await qualify(arguments)
      try emit(receipt)
    } catch let invalidation as PreviewObserverInvalidation {
      try? emit(ExactFixturePreviewObserverReceipt.invalid(invalidation))
    } catch {
      try? emit(
        ExactFixturePreviewObserverReceipt.invalid(
          .capture("unexpected internal exact-fixture preview observer failure")
        )
      )
    }
  }
  ...
  private static func qualify(
    _ arguments: ExactFixturePreviewObserverArguments
  ) async throws -> ExactFixturePreviewObserverReceipt {
    ...
    // This is the final live UI read before the synchronous packaged dispatch revalidates the
    // same fixture ID, canonical seed, and nine controls and delegates to production preview.
    let immediateFixtureReceipt = try accessibility.packagedFixtureReceipt(
      expectedDefinition: fixtureDefinition,
      expectedPhase: .configured
    )
    guard foreground.isIntact else { throw PreviewObserverInvalidation.candidateNotFrontmost }
    ...
    let accessibilityReceipt = try await waitForExactPreviewReceipt(
      accessibility,
      definition: fixtureDefinition
    )
    ...
    return ExactFixturePreviewObserverReceipt(
      observerVersion: exactFixturePreviewObserverSchemaVersion,
      status: "valid",
      target: .approved,
      fixture: ExactFixturePreviewAuthorityReceipt(
        fixtureId: immediateFixtureReceipt.fixtureId,
        fixtureDefinitionSha256: fixtureDefinitionSha256,
        worldSeed: immediateFixtureReceipt.worldSeed,
        controls: immediateFixtureReceipt.controls,
      ...
      dispatchBoundary: "exact-fixture-production-preview-key-dispatch",
      completionBoundary: "changed-preview-frame-then-exact-final-accessibility-receipt",
  ...
  private static func waitForBaseline(_ output: PreviewFrameOutput) async throws {
    ...
    throw PreviewObserverInvalidation.capture(
      "no complete baseline frame arrived before exact-fixture preview dispatch")
  }
  ...
  private static func waitForQualifyingFrame(
    ...
    throw PreviewObserverInvalidation.capture(
      "no complete changed post-dispatch frame satisfied the preview predicate")
  }
  ...
  private static func waitForExactPreviewReceipt(
    _ accessibility: AccessibilityObserver,
    definition: GatedAtlasFixtureDefinition
  ) async throws -> ExactFixturePreviewAccessibilityReceipt {
    for _ in 0..<200 {
      if let receipt = try? accessibility.exactFixturePreviewReceipt(
        expectedDefinition: definition
      ) { return receipt }
      try await Task.sleep(nanoseconds: 10_000_000)
    }
    throw PreviewObserverInvalidation.accessibility(
      "the final exact-fixture preview Accessibility receipt did not become complete")
  }
  ...
  private static func emit(_ receipt: ExactFixturePreviewObserverReceipt) throws {
...
private struct ExactFixturePreviewObserverArguments {
