...
### Option A — Store every output as vectors or polygons

This would make individual features easy to clip, but it would represent continuous
climate quantities as lossy geometry and make seam, pole, and adjacent-footprint
sampling unnecessarily dependent on topology reconstruction. It is rejected for
temperature, wind, moisture, and categorical fields.

### Option B — Use one shared quantized atlas lattice plus vector feature records

This stores continuous and categorical fields at the accepted M2 anchors, while ridges,
watersheds, rivers, and lakes remain meaningful vector entities with explicit portals.
It preserves exact shared samples and gives feature records the geometry needed for
clipping, identity, and downstream regional constraints. This is selected.

### Option C — Add a new binary persistence format before M3 integration

This could reduce package size for full-profile fields, but it would introduce a new
storage, checksum, migration, and recovery boundary before representative measurements
show that canonical JSON is insufficient. It is deferred; the existing project-owned
reader boundary is sufficient for the contract decision.
...
physical range. Vector fields use planet-centered Cartesian components, so their basis is
globally defined at the seam and both poles; they never store display-projection x/y or
latitude-dependent east/north components.
...
| `worldClimate.prevailingWinds` / world-surface singleton             | Quantized planet-centered Cartesian vector field with `x`, `y`, and `z` direction components, a `0.1 m/s` speed scalar, and a calm flag derived from speed. No projection-space arrows are authoritative.                                                                                                                                           | Latitude/planet coordinate; temperature; M2 coastline/water bodies; M3 mountain systems                                       | Aspect behavior 1, vector encoding 1, each direction component in signed normalized `2^24` ticks, speed in `0.1 m/s` ticks, canonical full-profile traversal. | Direction components are finite and bounded; non-calm direction has unit length within the declared quantization tolerance; calm is true exactly when speed is zero; the Cartesian basis gives one deterministic value at the seam and both poles.  |
...
| Control            | Exact values, default, and bounded meaning                                                                                          | First invalidated aspect       |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| Mountain character | enum `low` \| `varied` \| `rugged`; default `varied`; controls broad ridge prominence, not raw noise                                | `worldTerrain.mountainSystems` |
| Climate character  | enum `temperate` \| `varied` \| `extreme`; default `varied`; controls broad temperature/moisture realization within physical bounds | `worldClimate.temperature`     |

These are accepted enum controls with stable defaults; they do not expose noise
frequencies. #131 owns their schema encoding and validation, but may not change these
values or defaults without amending this ADR. A control change proposes a complete
dependency-closed patch and cannot replace accepted state until all rows' invariants pass.
A parent edit marks child context stale; it never rewrites the accepted child automatically.
