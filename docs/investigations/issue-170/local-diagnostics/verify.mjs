/** Read-only reproduction and byte verification of the retained local rejection evidence. */
...
for (const stage of ['before-repair', 'after-repair']) {
  ...
  assert.deepEqual(Buffer.from(reportBytes), await readFile(new URL(reportFile, HERE)), reportFile);
  assert.deepEqual(Buffer.from(image), await readFile(new URL(imageFile, HERE)), imageFile);
...
assert.deepEqual(
  Buffer.from(await json(manifest)),
  await readFile(new URL('manifest.json', HERE)),
  'manifest',
);
assert.equal(
  sha256(await readFile(new URL('../templates.mjs', HERE))),
  manifest.stages[1].sourceSha256,
  'frozen constructor',
);
process.stdout.write(
  'Verified both source snapshots, twelve constructor receipts, two planar PNGs, manifest dependencies, and retained bytes without writing evidence.\n',
);
