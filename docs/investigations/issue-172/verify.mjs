...
export async function verifyEvidence(
  ...
  assert(['issue-172-r1', 'issue-172-r2'].includes(manifest.revision), 'unsupported revision');
  assert(
  ...
  for (const [file, source] of Object.entries(snapshot)) {
    ...
    for (const imported of imports(source, file)) {
      ...
      if (
        file === 'run.mjs' &&
        imported.dynamic &&
        target === 'templates-r2.mjs' &&
        manifest.revision === 'issue-172-r1'
      )
        continue;
      assert(
  ...
  if (manifest.revision === 'issue-172-r2')
    for (const file of ['templates-r2.mjs', 'revision-r2.md'])
