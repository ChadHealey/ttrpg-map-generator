...
if (outputArgument !== undefined && outputArgument !== '--output') {
  throw new Error('Usage: node complete-m3-measurement.mjs [--output]');
}
...
function measure() {
  const temporaryRoot = mkdtempSync(resolve(tmpdir(), 'ttrpg-mapworld-issue-151-'));
  try {
    const repeats = [];
    for (let repeat = 1; repeat <= 2; repeat += 1) {
      ...
      const encodeWallMs = runVitest(
        ...
        {
          ISSUE_151_MEASUREMENT_MODE: 'encode',
          ISSUE_151_CANDIDATE_DIRECTORY: candidateDirectory,
          ISSUE_151_RESULT_PATH: encodeResultPath,
        },
      );
      const decodeWallMs = runVitest(
        ['apps/desktop/src/atlas-complete-m3-decode-measurement.test.ts'],
        {
          ISSUE_151_CANDIDATE_DIRECTORY: candidateDirectory,
          ISSUE_151_RESULT_PATH: decodeResultPath,
        },
      );
      repeats.push({
        repeat,
    ...
    return {
      ...
      command: 'node docs/investigations/issue-151/complete-m3-measurement.mjs --output',
      host: hostReceipt(),
      toolchain: {
      ...
      measurementSources: [
        'apps/desktop/src/atlas-physical-accepted-state.integration.test.ts',
        'apps/desktop/src/atlas-complete-m3-decode-measurement.test.ts',
        'docs/investigations/issue-138/macos-results.json',
        'docs/investigations/issue-151/complete-m3-measurement.mjs',
        'packages/persistence/src/mapworld-v2-aspect-dto-schema.ts',
        'packages/persistence/src/mapworld-v2-codec.ts',
        'packages/persistence/src/mapworld-v2-label-dto-schema.ts',
      ...
      source: {
        fixtureId: 'complete-m3-atlas-plus-supplied-south-pole-context',
        physicalAspectCount:
          first.encode.framedEvidence.filter(({ aspectName }) => aspectName.startsWith('world'))
            .length - first.encode.nameCount,
        logicalFieldCount: 9,
        nameCount: first.encode.nameCount,
        placementCount: first.encode.placementCount,
        manualOverrideCount: first.encode.manualOverrideCount,
      ...
      canonicalEvidence: {
        ...
        physicalFingerprints: first.encode.physicalFingerprints,
        labelAcceptedStateSha256: first.encode.labelAcceptedStateSha256,
        inheritedContextChecksum: first.encode.inheritedContextChecksum,
      },
      physicalContextV2Compatibility: first.encode.physicalContextCompatibility,
      repeats: repeats.map((item) => ({
        ...
        nameCount: item.decode.nameCount,
        placementCount: item.decode.placementCount,
        manualOverrideCount: item.decode.manualOverrideCount,
        manifestSha256: item.encode.manifestSha256,
        labelAcceptedStateSha256: item.decode.labelAcceptedStateSha256,
        inheritedContextChecksum: item.decode.inheritedContextChecksum,
      })),
      linuxCorroboration: {
        status: 'deferred',
        requirement:
          'Matching canonical bytes, checksums, fingerprints, decoded state, and limit results are required before production v2 writer release.',
      },
    };
  } finally {
...
function validateRepeats(repeats) {
  ...
  for (const item of repeats) {
    if (
      ...
      item.decode.decodedAspectCount !== item.encode.framedEvidence.length ||
      item.decode.physicalAspectCount !== 9 ||
      item.decode.nameCount !== item.encode.nameCount ||
      item.decode.placementCount !== item.encode.placementCount ||
      item.decode.manualOverrideCount !== item.encode.manualOverrideCount ||
      JSON.stringify(item.encode.framedEvidence) !== JSON.stringify(item.decode.framedEvidence) ||
      JSON.stringify(item.encode.physicalFingerprints) !==
        JSON.stringify(item.decode.physicalFingerprints) ||
      item.encode.labelAcceptedStateSha256 !== item.decode.labelAcceptedStateSha256 ||
    ...
    if (
      item.encode.physicalContextCompatibility.manifestSha256 !==
      item.encode.physicalContextCompatibility.baselineManifestSha256
    ) {
      throw new Error(
        `Repeat ${String(item.repeat)} changed the physical/context-only v2 package.`,
      );
    }
  }
  for (const key of [
    'files',
    'framedEvidence',
    'physicalFingerprints',
    'labelAcceptedStateSha256',
...
function repositoryFilesystem() {
  ...
  const device = optionalCommandOutput('/bin/df', [repositoryRoot])
    ?.split('\n')
    .at(-1)
    ?.trim()
    .split(/\s+/)[0];
  const mountLine = optionalCommandOutput('/sbin/mount', [])
    ?.split('\n')
...
function macModelIdentifier() {
  const output = optionalCommandOutput('/usr/sbin/system_profiler', [
    'SPHardwareDataType',
    '-json',
  ]);
