...
#[test]
#[ignore = "run through the issue #121 no-launch Swift/Rust interoperability gate"]
fn rust_swift_production_authority_zero_command_qualification() {
    let swift_client = std::env::var_os("ISSUE121_SWIFT_CLIENT")
        .map(PathBuf::from)
        .expect("ISSUE121_SWIFT_CLIENT must name the freshly compiled Swift client");
    let private_directory = PrivateDirectory::create();
    ...
    let ready = authority.frontend_ready().expect("production READY");
    write_fragmented(&mut stream, &encode(&ready).expect("READY bytes"));
    let mut post_ready_bytes = Vec::new();
    stream
        .read_to_end(&mut post_ready_bytes)
        .expect("qualification controller close");
    assert!(
        post_ready_bytes.is_empty(),
        "zero-command qualification emitted bytes after READY"
    );
    authority.close();
    drop(stream);
    ...
    let output = child
        .wait_with_output()
        .expect("Swift qualification result");
    assert!(
        output.status.success(),
        "Swift qualification failed with sanitized output"
    );
    assert!(output.stderr.is_empty());
    let public_result = String::from_utf8(output.stdout).expect("UTF-8 public result");
    assert!(public_result.contains(r#""status":"valid""#));
    assert!(public_result.contains(r#""commandCount":0"#));
    assert!(public_result.contains(r#""terminalCleanupSucceeded":true"#));
    assert!(
        !socket_path.exists(),
        "endpoint socket survived qualification"
    );
    assert!(
        !private_directory.0.exists(),
        "private endpoint directory survived qualification"
    );
    for private_value in [
        socket_path.to_string_lossy().as_ref(),
        &hex(&SESSION),
