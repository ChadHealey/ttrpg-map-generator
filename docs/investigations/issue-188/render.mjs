...
export function render(reports) {
  ...
  if (panels.length !== 2) throw new Error('Exactly two fixed component panels required');
  ...
  for (let k = 0; k < 2; k++) {
    ...
    for (let y = 0; y < height; y++)
      for (let x = 0; x < width / 2; x++)
        pixels[y * width + (k * width) / 2 + x] = pointInPolygon(
          [(x / (width / 2) - 0.5) * span, (0.5 - y / height) * span],
          polygon,
        )
          ? 1
          : 0;
