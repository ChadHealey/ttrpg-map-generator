...
const FIXED = [
  ...
  'verify.mjs',
  'design.md',
  'independent-literal-review.md',
  'independent-source-review.md',
  'harness-plan.md',
].map((p) => PREFIX + p);
export function stageEntry(stage) {
  assert(['state-1', 'state-2'].includes(stage), 'Only two declared states');
...
export async function capture(stage) {
  ...
  const second =
    stage === 'state-2'
      ? [
          ...
          'states/state-2/authorization.json',
      ...
      : [];
  for (const file of [...FIXED, ...second, 'package.json', 'pnpm-lock.yaml'])
  ...
  return {
    ...
    snapshot: ordered,
    manifest: {
      revision: 'issue-190-local-r1',
      stage,
      entry,
      phaseSlots: { initial: 60, repeat: 60, replay: 60 },
