/** First of at most two immutable issue183 nine-row comparisons. */
...
import { constructOwners } from './recipes/recipe-3/templates.mjs';
...
async function main() {
  ...
  const revision = 'issue-183-world-r1-recipe3';
  const comparison = mode.startsWith('--compare');
  const output = resolve(target);
  if (comparison && output !== resolve(HERE, 'comparison-r1'))
    throw new Error('First matrix must use comparison-r1');
  if (!output.startsWith(HERE) || output === HERE.slice(0, -1))
    throw new Error('Evidence must use a new revision directory inside issue-183.');
