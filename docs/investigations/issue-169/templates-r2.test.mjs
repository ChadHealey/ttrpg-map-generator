...
test('balanced six-body and high-abundance local certificates fit unchanged shares', () => {
  const cases = [
    {
  ...
  for (const input of cases) {
    ...
    if (input.controls.continentDistribution === 'balanced')
      assert.ok(result.owners.every((o) => o.radius < (Math.PI / 2 - 0.05) / 2));
  }
});
