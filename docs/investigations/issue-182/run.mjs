...
const BASE = new URL('.', import.meta.url),
  DIR = new URL('evidence/', BASE),
  prefix = '../issue-182/';
const entries = [
  prefix + 'templates.mjs',
...
async function load() {
  ...
  for (const path of ['run.mjs', '../issue-177/local-evidence.mjs'])
    assert.equal(
  ...
  const template = await loadSnapshot(entries[0], snapshot),
    ...
    declared = await loadSnapshot('../issue-180/corpus.mjs', snapshot);
  assert.deepEqual(
    manifest.probes,
    declared.corpus().filter((p) => p.cohort === 'additional-default'),
  );
  assert.equal(manifest.probes.length, 128);
  const runtime = {
    ...template,
    ...placement,
    ...certificate,
...
if (mode === '--prepare') {
  const probes = corpus().filter((p) => p.cohort === 'additional-default'),
    previous = JSON.parse(
  ...
  assert.equal(probes.filter((p) => p.cohort === 'control').length, 0);
  assert.equal(new Set(probes.map((p) => p.input.id)).size, 128);
  ...
  await save('manifest.json', {
    revision: 'issue-182-large-primary-recovery-r1',
    scope:
      'Construction and bounded placement only; no raster or production/semantic qualification',
    ordering: 'Exact issue-180 first128 additional-default probes; no additional controls',
    probes,
