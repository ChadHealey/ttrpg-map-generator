  ...
  probes().map(
    async (p) =>
      JSON.parse(
        gunzipSync(
          await readFile(
            new URL(`../issue-183/readiness-recipe-3/${p.input.id}.json.gz`, import.meta.url),
          ),
        ),
      ).result,
  ),
...
test('each named recovered row must retain the actual revised large layout', () => {
  for (const id of ['default-001', 'default-004', 'default-006']) {
    const changed = structuredClone(rows),
      row = changed.find((r) => r.probe.input.id === id);
    for (const owner of row.construction.owners) if (owner.primary) owner.candidate.layoutIndex = 1;
    const gate = assessRows(changed);
    assert.equal(gate.readyForComparison, false);
    assert(gate.failures.some((f) => f.code === 'missing-recovered-layout' && f.id === id));
  }
});
