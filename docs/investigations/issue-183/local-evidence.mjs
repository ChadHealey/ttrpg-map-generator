...
assert(['--write', '--verify'].includes(mode) && /^(baseline|recipe-[123])$/.test(stage));
const dir = new URL(`local-diagnostics/${stage}/`, BASE);
const entries = ['a', 'b', 'c'].map((name) => prefix + `recipes/${stage}/layout-${name}.mjs`);
  ...
  for (let l = 0; l < entries.length; l++) {
    ...
    for (const quota of quotas)
      for (const anatomy of anatomies) {
        const split = buildCoast('local-primary', { anatomy, variation: 0 });
        ...
        reports.push({ layout: ['A', 'B', 'C'][l], quota, anatomy, candidate, certificate });
      }
  }
  const panels = reports.filter((r) => r.quota === quotas[0] && r.anatomy.every((x) => x === 0));
  const width = 900,
    height = 320,
    span = 2.5,
...
if (mode === '--write') {
  ...
  console.log(JSON.stringify({ stage, verified: true, receipts: 45, images: 1 }));
}
