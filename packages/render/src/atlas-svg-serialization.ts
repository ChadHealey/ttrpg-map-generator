...
function glyphDefinitionLines(scene: AtlasRenderScene): string[] {
  return (scene.vectorLabels?.definitions ?? []).map((glyph) => {
...
function renderAtlasSvgLabelNode(node: RenderNode, label: AtlasVectorLabelNode): string {
...
function atlasSvgSerializedByteLength(input: AtlasSvgSerializationInput): number {
