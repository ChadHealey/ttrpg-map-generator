...
import { coverage, gridBytes, localGate } from './run.mjs';

test('the comparison gate cannot pass when a constructor drops owner slots', async () => {
  const empty = await localGate(() => ({ ok: true, owners: [] }));
  assert.equal(empty.completeCohortPassed, false);
  const short = await localGate((input) => ({
    ok: true,
    owners: Array.from({ length: input.controls.continentCountIntent - 1 }, (_, i) => ({
      id: `owner-${i}`,
      certificate: { ok: true },
      candidate: { primary: true },
    })),
  }));
  assert.equal(short.completeCohortPassed, false);
});
