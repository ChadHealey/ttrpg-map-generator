...
Accepted-record validation executes the same core-owned version-1 land thresholds, island-group
budget/geometry/order, clearance segmentation, enclosure, and basin-root rules as generation.
Hardening that validator to reject records that never conformed to these already-accepted rules
does not change generated bytes, fingerprints, identities, thresholds, or ordering, so it does not
increment a compatibility version or authorize fixture regeneration.
