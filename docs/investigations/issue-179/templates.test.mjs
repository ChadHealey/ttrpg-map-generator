...
test('all six complete local owner sets are deterministic, paid and within fixed budgets', () => {
  for (const input of inputs) {
    ...
    for (const owner of result.owners) {
      ...
      assert.deepEqual(
        certifyCandidate(owner.candidate, { quota: owner.quota, ...CERTIFICATE_OPTIONS }),
        owner.certificate,
      );
...
test('three compact subordinate coasts retain explicit witnesses and paid members', () => {
  const owners = inputs.flatMap((i) => constructOwners(i).owners.filter((o) => !o.primary));
  assert.deepEqual([...new Set(owners.map((o) => o.candidate.layoutIndex))].sort(), [0, 1, 2]);
...
test('the explicit approved far-crosscut mode is fixed for every constructor candidate', () => {
  assert.deepEqual(CERTIFICATE_OPTIONS, {
    nominalClearance: 0.05,
    collarWidthUpperMode: 'root-and-far',
  });
  assert(Object.isFrozen(CERTIFICATE_OPTIONS));
  for (const input of inputs)
    for (const owner of constructOwners(input).owners) {
      assert.equal(owner.candidate.collarWidthUpperMode, 'root-and-far');
      assert.equal(owner.certificate.metrics.collarWidthUpperMode, 'root-and-far');
      for (const p of owner.certificate.metrics.roles.filter((r) => r.kind === 'peninsula')) {
        assert.equal(p.widthUpperWitness, 'far');
        assert(p.widthUpperRoot > 0.16);
      }
    }
});
