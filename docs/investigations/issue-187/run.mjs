...
async function reconstruct(captured) {
  ...
  const first = evaluator.evaluate(layout.buildCoast, declared),
    second = evaluator.evaluate(layout.buildCoast, corpus.inputs());
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  assert(
    ...
    'Use --record/--verify state-1 or state-2',
  );
  console.log(
    JSON.stringify(
